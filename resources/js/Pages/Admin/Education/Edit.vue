<template>
    <div>
      <div class="flex items-center gap-2 mb-6">
        ...
        <h1 class="text-2xl font-bold">Edit Konten Edukasi</h1>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Perbarui informasi untuk konten "{{ educationContent.title }}".
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form @submit.prevent="submitForm" class="space-y-6">
              ...
              <div class="space-y-4">
                ...
                <div>
                  ...
                  />
                    ...
                    <p class="text-xs text-muted-foreground">
                      Upload gambar thumbnail baru (format: JPEG, PNG, JPG, GIF - maksimal 2MB) atau biarkan kosong untuk tidak mengubah gambar
                    </p>
                    
                    <!-- Current Image Display -->
                    <div v-if="!imagePreview && educationContent.image" class="space-y-2">
                    ...
                    <div v-if="!imagePreview && !educationContent.image" class="h-32 w-48 bg-muted rounded-lg border-2 border-dashed border-muted-foreground/25 flex items-center justify-center">
                      <div class="text-center">
                        <ImageIcon class="h-8 w-8 mx-auto text-muted-foreground/50 mb-2" />
                        <p class="text-sm text-muted-foreground">Upload gambar thumbnail</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div v-if="isInfografis">
                  ...
                  />
                    ...
                    <p class="text-xs text-muted-foreground">
                      Upload file infografis baru (format: JPEG, PNG, JPG, GIF, PDF - maksimal 5MB) atau biarkan kosong untuk tidak mengubah
                    </p>
                    
                    <!-- Current Infographic Display -->
                    <div v-if="!infographicPreview && educationContent.infographic_url" class="space-y-2">
                      <p class="text-sm font-medium">Infografis Saat Ini:</p>
                      <img 
                        :src="educationContent.infographic_url" 
                        alt="Current Infographic" 
                        class="h-32 w-48 object-cover rounded-lg border"
                      />
                    </div>
                    
                    <!-- New Infographic Preview -->
                    <div v-if="infographicPreview" class="space-y-2">
                      <p class="text-sm font-medium">Preview Infografis Baru:</p>
                      <div class="relative inline-block">
              ...
              <div class="space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem :value="true">Publikasikan</SelectItem>
            ...
            <div>
              ...
              <div class="border border-border rounded-md p-4">
                <div v-if="imagePreview || educationContent.image" class="mb-4">
                  <p class="text-sm text-muted-foreground mb-2">Thumbnail:</p>
                  <img :src="imagePreview || educationContent.image" alt="Preview" class="h-48 w-full object-cover rounded-md" />
                </div>
                ...
                <div v-if="form.type === 'Infografis' && (infographicPreview || educationContent.infographic_url)" class="mb-4">
                  <p class="text-sm text-muted-foreground mb-2">Infografis:</p>
                  <div v-if="infographicPreview && infographicPreview.type === 'image'">
                    <img :src="infographicPreview.url" alt="Infografis Preview" class="max-w-full rounded-md" />
                  </div>
                  <div v-else-if="infographicPreview && infographicPreview.type === 'file'" class="p-4 bg-muted rounded-md">
                    <p class="text-sm">File: {{ infographicPreview.name }}</p>
                  </div>
                  <div v-else-if="educationContent.infographic_url">
                    <img :src="educationContent.infographic_url" alt="Infografis Preview" class="max-w-full rounded-md" />
                  </div>
                </div>
  ...
  const form = reactive({
    ...
    infographic: null, // Will contain new uploaded file if any
    published: props.educationContent.published === true, // Ensure boolean
    errors: {},
    processing: false
  });
  ...
  const submitForm = () => {
    ...
    router.post(route('admin.education.update', props.educationContent.id), formData, {
      forceFormData: true,
      onError: (errors) => {
