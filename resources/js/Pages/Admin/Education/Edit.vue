<template>
          ...
          <form @submit.prevent="submitForm" class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="space-y-4">
                ...
                <div>
                  <Label for="views">Jumlah Pembaca</Label>
                  <Input 
                    id="views" 
                    v-model="form.views" 
                    type="number" 
                    min="0" 
                    :class="{ 'border-destructive': form.errors.views }"
                    required
                  />
                  <p v-if="form.errors.views" class="text-destructive text-sm mt-1">{{ form.errors.views }}</p>
                </div>
              </div>
            </div>
  ...
  const initialTitle = ref(props.educationContent.title);
  const initialDescription = ref(props.educationContent.description);
  const initialType = ref(props.educationContent.type);
  const initialImage = ref(props.educationContent.image);
  const initialContent = ref(props.educationContent.content);
  const initialVideoUrl = ref(props.educationContent.video_url || '');
  const initialInfographicUrl = ref(props.educationContent.infographic_url || '');
  const initialTags = ref((props.educationContent.tags || []).join(', '));
  const initialPublished = ref(props.educationContent.published !== false);
  const initialViews = ref(props.educationContent.views || 0);
  ...
  const form = useForm({
    ...
    video_url: initialVideoUrl.value,
    infographic_url: initialInfographicUrl.value,
    tags: initialTags.value,
    published: initialPublished.value,
    views: initialViews.value,
  });
  ...
  const submitForm = () => {
    ...
    form.put(route('admin.education.update', props.educationContent.id), {
      onSuccess: () => {
        router.visit(route('admin.education.show', props.educationContent.id));
      },
    });
  };
  </script>
